...
from finance.common.model import Provider
...
def test_normalize_providers_basic(unwrap):
    ...
    assert providers["yahoo"] == Provider(name="yahoo", **default_params)
    assert providers["ecb"] == Provider(
        name="ecb",
        **default_params | {"timezone": "Europe/Berlin", "intraday_history_limit": "10d", "daily_history_limit": "20y"},
    )
    assert providers["fred"] == Provider(name="fred", **(default_params | {"daily_series_type": "value"}))
