...
def test_extract_candles_valid_output_structure(provider, unwrap):
    ...
    result = MeasurementResult.ok_payload("x",{
        "timestamp": [1000],
        "indicators": {
            "quote": [
                {
...
def test_extract_candles_empty_mapping(provider):
    ...
    assert "Unknown fields requested: ['hoog', 'laag']. Supported: ['open', 'high', 'low', 'close', 'volume']" in candles.reason


def test_extract_candles_skips_invalid(provider, unwrap):

    result = MeasurementResult.ok_payload("u", {
        "timestamp": [1000],
        "indicators": {
            "quote": [
                {
...
def test_extract_candles_filters_today(provider, unwrap):
    ...
    result = MeasurementResult.ok_payload("t", {
